...
class BackupKind
  ...
  def should_be_used?(archive, date = Date.today)
    return true unless @runs_on_each
    return true unless archive.exists?(name)
    return @runs_on_each.include?(date)
  end

  def should_run_on_each runt_spec
    @runs_on_each = runt_spec
  end

  def should_run_on? date
    @runs_on_each.include? date
  end

  def execute(archive)
    return false unless should_be_used?(archive, archive.run_date)
    archive.create(name)
